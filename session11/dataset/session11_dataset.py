...
def get_album_transforms(norm_mean,norm_std):
    ...
    album_train_transform = A.Compose([
                                          ...
                                          A.Cutout(1, 8, 8, norm_mean, False, p=0.4),
                                          A.Normalize(
                                             mean=norm_mean,
                                              std=norm_std, ),
...
def get_dataloaders(train_set,test_set):
    ...
    dataloader_args = dict(shuffle=True, batch_size=512, num_workers=4, pin_memory=True) if cuda else dict(shuffle=True, batch_size=64, num_workers=1)
